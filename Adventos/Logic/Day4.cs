...
namespace Adventos.Logic
{
    ...
    public class Day4
    {
        ...
        public Day4()
        {
            var input = FileUtils.ReadRaw("day4.txt");
            _drawNumbers = input.Split("\n")[0].Split(',').Select(x => int.Parse(x)).ToList();
            var cards = input.Replace("  ", " ").Split("\n\n").ToArray()[1..];
            _cardCount = cards.Count();
            _bingoNumbers = cards.SelectMany((x, i) => x.Split("\n").SelectMany((y, j) => y.Trim().Split(" ").Select((z, k) => new BingoNumber { Number = string.IsNullOrEmpty(z) ? 0 : int.Parse(z), Coord = (X: k, Y: j), Card = i }))).ToList();
        }
